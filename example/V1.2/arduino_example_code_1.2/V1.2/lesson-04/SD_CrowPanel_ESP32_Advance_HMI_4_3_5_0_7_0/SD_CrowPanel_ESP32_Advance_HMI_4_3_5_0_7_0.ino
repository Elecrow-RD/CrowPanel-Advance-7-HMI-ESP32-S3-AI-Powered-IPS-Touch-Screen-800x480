...
/* ================= LVGL 8.3.3 性能监视器配置（必须写在lvgl.h之后，或修改lv_conf.h） ================= */
// 完全采用 LVGL 官方 perf monitor（FPS / CPU 统计与显示）
#undef LV_USE_PERF_MONITOR
#define LV_USE_PERF_MONITOR 1  // 开启官方性能监视器

#undef LV_PERF_MONITOR_POS
#define LV_PERF_MONITOR_POS 3  // 显示位置：右下角

#undef LV_USE_CPU_USAGE
#define LV_USE_CPU_USAGE 1     // 开启CPU使用率统计
...
int displayImage(fs::FS &fs, String filename, int width, int height)
{
    ...
    // 跳过BMP文件头（54字节）到像素数据区
    f.seek(54); 
    
    uint8_t RGB[3 * width]; // 存储一行的RGB数据
    uint8_t RGB_flipped[3 * width]; // 存储左右翻转后的RGB数据
    
    // ========== 关键修正：移除上下翻转，只保留左右翻转 ==========
    for (int row = 0; row < height; row++) 
    {
      // 直接读取当前行（不再计算height-1-row，避免上下翻转）
      ...
      for (int col = 0; col < width; col++) {
        int src_pos = col * 3;                     // 原像素位置
        int dst_pos = (width - 1 - col) * 3;       // 左右翻转后的位置
        // 复制RGB数据（仅左右交换）
        RGB_flipped[dst_pos]     = RGB[src_pos];     // R通道
        RGB_flipped[dst_pos + 1] = RGB[src_pos + 1]; // G通道
        RGB_flipped[dst_pos + 2] = RGB[src_pos + 2]; // B通道
      }
