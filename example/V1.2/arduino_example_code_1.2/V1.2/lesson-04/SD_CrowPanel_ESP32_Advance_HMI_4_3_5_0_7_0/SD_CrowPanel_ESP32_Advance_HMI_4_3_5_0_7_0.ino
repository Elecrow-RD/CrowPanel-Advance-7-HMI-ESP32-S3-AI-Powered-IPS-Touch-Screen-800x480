...
int displayImage(fs::FS &fs, String filename, int width, int height)
{
    ...
    // Skip BMP file header (54 bytes) to reach pixel data area
    f.seek(54); 
    
    uint8_t RGB[3 * width]; // Store one row of RGB data
    uint8_t RGB_flipped[3 * width]; // Store horizontally flipped RGB data
    
    // ========== Key Fix: Remove vertical flip, keep horizontal flip only ==========
    for (int row = 0; row < height; row++) 
    {
      // Read current row directly (no longer calculate height-1-row to avoid vertical flip)
      f.seek(54 + 3 * width * row); 
      
      // Read pixel data for current row
      if (f.read(RGB, 3 * width) != 3 * width) {
        Serial.println("Error reading image data at row: " + String(row));
        break;
      }
      
      // Only perform horizontal mirror correction
      for (int col = 0; col < width; col++) {
        int src_pos = col * 3;                     // Original pixel position
        int dst_pos = (width - 1 - col) * 3;       // Horizontally flipped position
        // Copy RGB data (swap left/right only)
        RGB_flipped[dst_pos]     = RGB[src_pos];     // R channel
        RGB_flipped[dst_pos + 1] = RGB[src_pos + 1]; // G channel
        RGB_flipped[dst_pos + 2] = RGB[src_pos + 2]; // B channel
      }
      
      // Display corrected row data
      gfx.pushImage(0, row, width, 1, (lgfx::rgb888_t *)RGB_flipped);
    }
