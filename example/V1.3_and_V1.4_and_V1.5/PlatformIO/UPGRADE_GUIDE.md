...
6. Confirm that both 128,000-byte LVGL partial buffers are allocated from PSRAM.

The firmware aborts with a serial message if either display buffer cannot be
allocated. The display uses two 80-line partial buffers. The two GPIO controls
use fixed-style clickable objects instead of themed buttons, so their pressed
and released states do not redraw the single RGB framebuffer. LVGL is notified
only after an actual framebuffer write is complete.
